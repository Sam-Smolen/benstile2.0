<?php include 'includes/header.php';?>
  <!-- END  OF NAVBAR SECTION -->

   <!-- START LEARN MORE SECTION-->
   <section id="learn-more" class="learn-more">
  <div class="learn-about__section">
    <div class="learn-about__container">
      <h1 class="learn__about--header text-start">Custom Bathroom Design</h1>
        <div class="learn-about__heading--line mb-5"></div>
        <div class="learn-about__content">
            <div class="learn-about__text text-start">
            <h2 class="display-5">Gallery | Custom Bathroom Remodels and Additions</h2>
            <p class="mt-3 pt-3"></p>
        </div>
        <button class="btn btn-primary text-white text-capitalize">
          <a href="contact.php" class="text-white">Schedule an appointment</a>
        </button>
        </div>
    </div>
   </div>
  </section>
  <!-- END LEARN MORE SECTION-->

  <!-- START GALLERY IMAGES-->
  <section id="learn-more-portfolio" class="learn-more-portfolio">
    <!--svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320"><path fill="#fff" fill-opacity="1" d="M0,128L30,138.7C60,149,120,171,180,154.7C240,139,300,85,360,101.3C420,117,480,203,540,208C600,213,660,139,720,128C780,117,840,171,900,197.3C960,224,1020,224,1080,192C1140,160,1200,96,1260,90.7C1320,85,1380,139,1410,165.3L1440,192L1440,0L1410,0C1380,0,1320,0,1260,0C1200,0,1140,0,1080,0C1020,0,960,0,900,0C840,0,780,0,720,0C660,0,600,0,540,0C480,0,420,0,360,0C300,0,240,0,180,0C120,0,60,0,30,0L0,0Z"></path></svg-->
    <!--div class="learn__more--gallery">
      <div class="row text-center mt-5">
        <h1 class="display-3 fw-bold text-capitalize">latest Work</h1>
        <div class="heading-line"></div>
        <p class="lead">
          check out some of our latest projects!
        </p>
      </div-->
      <!-- FILTER BUTTONS -->
      <!--div class="row mt-5 mb-4 g-3 text-center">
        <div class="col-md-12">
          <button class="btn btn-outline-primary" type="button">All</button>
          <button class="btn btn-outline-primary" type="button">Tile Work</button>
          <button class="btn btn-outline-primary" type="button">Bathroom design</button>
          <button class="btn btn-outline-primary" type="button">Kitchen design</button>
        </div>
      </div-->
      <!-- START PORTFOLIO ITEMS -->
      <div class="row">
        <div class="col-lg-4 col-md-6">
          <div class="portfolio-box shadow mt-0 pt-0">
            <img src="images/bathroom.jpg" alt="reed1" title="custom tile design" class="img-fluid">
            <div class="portfolio-info">
             <div class="caption">
              <h4>Reed 2022</h4>
              <p>Custom tile design</p>
            </div>
          </div>
        </div>
        <h5 class="gallery-caption">Hello</h5>
      </div> 
        <div class="col-lg-4 col-md-6">
          <div class="portfolio-box shadow">
            <img src="images/portfolio/reed2.jpg" alt="reed 2" title="custom tile design" class="img-fluid">
            <div class="portfolio-info">
             <div class="caption">
              <h4>Reed 2022</h4>
              <p>Custom tile design</p>
            </div>
          </div>
        </div>
        <h5 class="gallery-caption">Hello</h5>
      </div> 
        <div class="col-lg-4 col-md-6">
          <div class="portfolio-box shadow">
            <img src="images/portfolio/reed3.jpg" alt="reed 3" title="custom tile design" class="img-fluid">
            <div class="portfolio-info">
             <div class="caption">
              <h4>Reed 2022</h4>
              <p>Custom tile design</p>
            </div>
          </div>
        </div>
        <h5 class="gallery-caption">Hello</h5>
      </div> 
        <div class="col-lg-4 col-md-6">
          <div class="portfolio-box shadow">
            <img src="images/portfolio/breer1.jpeg" alt="breer1" title="custom kitchen design" class="img-fluid">
            <div class="portfolio-info">
             <div class="caption">
              <h4>Breer 2022</h4>
              <p>Custom kitchen design</p>
            </div>
          </div>
        </div>
        <h5 class="gallery-caption">Hello</h5>
      </div> 
        <div class="col-lg-4 col-md-6">
          <div class="portfolio-box shadow">
            <img src="images/portfolio/breer2.jpg" alt="breer3" title="custom kitchen design" class="img-fluid">
            <div class="portfolio-info">
             <div class="caption">
              <h4>Breer 2022</h4>
              <p>Custom kitchen design</p>
            </div>
          </div>
        </div>
        <h5 class="gallery-caption">Hello</h5>
      </div> 
        <div class="col-lg-4 col-md-6">
          <div class="portfolio-box shadow">
            <img src="images/portfolio/johnson1.jpg" alt="johnson 1" title="custom tile design" class="img-fluid">
            <div class="portfolio-info">
             <div class="caption">
              <h4>Johnson 2022</h4>
              <p>Custom tile design</p>
            </div>
          </div>
        </div>
        <h5 class="gallery-caption">Hello</h5>
      </div> 
        <div class="col-lg-4 col-md-6">
          <div class="portfolio-box shadow">
            <img src="images/portfolio/johnson2.jpg" alt="johnson 2" title="custom tile design" class="img-fluid">
            <div class="portfolio-info">
            <div class="caption">
              <h4>Johnson 2022</h4>
              <p>Custom tile design</p>
            </div>
          </div>
        </div>
        <h5 class="gallery-caption">Hello</h5>
      </div> 
        <div class="col-lg-4 col-md-6">
          <div class="portfolio-box shadow">
            <img src="images/portfolio/motley.jpg" alt="motley" title="custom tile design" class="img-fluid">
            <div class="portfolio-info">
             <div class="caption">
              <h4>Motley 2022</h4>
              <p>Custom tile design</p>
            </div>
          </div>
        </div>
        <h5 class="gallery-caption">Hello</h5>
      </div> 
        <div class="col-lg-4 col-md-6">
          <div class="portfolio-box shadow">
            <img src="images/portfolio/peterson.jpg" alt="peterson kitchen" title="custom kitchen design" class="img-fluid">
            <div class="portfolio-info">
             <div class="caption">
              <h4>Peterson 2022</h4>
              <p>Custom kitchen design</p>
            </div>
          </div>
        </div>
        <h5 class="gallery-caption">Hello</h5>
      </div> 
    </div>
  </div>
  <!--svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320"><path fill="#fff" fill-opacity="1" d="M0,128L30,138.7C60,149,120,171,180,154.7C240,139,300,85,360,101.3C420,117,480,203,540,208C600,213,660,139,720,128C780,117,840,171,900,197.3C960,224,1020,224,1080,192C1140,160,1200,96,1260,90.7C1320,85,1380,139,1410,165.3L1440,192L1440,320L1410,320C1380,320,1320,320,1260,320C1200,320,1140,320,1080,320C1020,320,960,320,900,320C840,320,780,320,720,320C660,320,600,320,540,320C480,320,420,320,360,320C300,320,240,320,180,320C120,320,60,320,30,320L0,320Z"></path></svg-->
  </section>
  <!-- END GALLERY IMAGES-->
  <?php include 'includes/learnmorefooter.php';?>
 